...
def limits(u, z, wvl):
    # u is delta u
    return (1/wvl) * np.sqrt((2 * u * z)**2 + 1) **-1
...
class FFT:
    ...
    def Cal(self, color):
        ...
        Ref = Refwave(wvl, zzz * ratio, self.thetaX * ratio, self.thetaY * ratio)
        #Ref_re = self.normalize(Ref, type='real')
        #Ref_im = self.normalize(Ref, type='phase')
        #Ref = np.complex(Ref_re, Ref_im)
        result *= Ref
        return result
    ...
    def ASM(self, color):
        ...
        ph = np.exp(1j * phase)
        ph *= np.exp(1j*k(wvl) * self.zz)
        ph *= image
        CH = self.fft(ph)
        CH = CH * asm_kernel(wvl, self.zz)
        result = self.ifft(CH)
        result = result[h: (2 * h), w: (2 * w)]
        result *= Refwave(wvl, self.zz, self.thetaX * ratio, self.thetaY* ratio)
        return result
...
if __name__ == '__main__':
    fs = FFT('DICE_RGB1.bmp')
    fs.getRGBImage(fs.ASM('red'), fs.ASM('green'), fs.ASM('blue'), '201012 test1.bmp', type='angle')
    #fs.getRGBImage(fs.ASM('red'), fs.ASM('green'), fs.ASM('blue'), '200923 2D ASMFFT offaxis new4.bmp', type='angle')
